...
OTUs_asd = "../../data/MACADAM/ListOTUs/Genus_Fam_Ord_Phylum_complete_name_Autism.txt"
ListOtu <- read.csv2(OTUs_asd, header=FALSE)
...
# take the last 2 elements (words) of each missingOTU...
...
path_asd <- "../../data/MACADAM/Filebase/Autism" # ASD
files_to_read2 <- list.files(path=path_asd, pattern="\\.csv$", full.names=TRUE)
# scroll through all OTUs tables
for(namefile in files_to_read2){
  ...
  # Update patient variable names adding numerical labels
  ...
  fileLogabund <- fileLogabund %>%
	                dplyr::select(-disease_name) # drop column from fileLogabund
  ...
  #write.table(fileabundLong, paste("fileabundlong/fal", tail(unlist(strsplit(namefile, "/")),1), sep=""), dec=".", sep=";", quote=FALSE)
  ...
  #write.table(fileabundLong2, paste("fileabundlong2/fal2", tail(unlist(strsplit(namefile, "/")),1), sep=""), dec=".", sep=";", quote=FALSE)
  print(namefile)
  print(dim(fileabundLong2))

  # The result has to be numeric and not char, otherwise the matrix product gives error
  ...
  pathwaysfileWideMatrix2 <- pathwaysfileWideMatrix[, XXX] # subset with XXX columns
  pathwaysfileWideMatrix2 <- pathwaysfileWideMatrix2[rowSums(pathwaysfileWideMatrix2)>0,]

  bindme <- rbind(pathwaysfileWideMatrix2, fileabund2[, XXX]) # join w/ subset with XXX columns
